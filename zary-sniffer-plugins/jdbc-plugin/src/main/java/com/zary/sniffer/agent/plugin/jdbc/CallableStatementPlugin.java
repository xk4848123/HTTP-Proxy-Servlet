...
/**
 * 拦截插件：CallableStatement插件
 * <p>
 * 1.CallableStatement插件是jdbc中执行存储过程查询器
 * 2.通过拦截excute相关函数解析数据库执行参数
 */
public class CallableStatementPlugin extends AbstractPlugin {
    private static final String HANDLER = "com.zary.sniffer.agent.plugin.jdbc.handler.CallableStatementHandler";

    @Override
    public ElementMatcher<TypeDescription> getPluginTypeMatcher() {
        return ElementMatchers.isSubTypeOf(java.sql.CallableStatement.class)
